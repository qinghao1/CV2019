...
function [] = surfaceRender(pointcloud, M, Mean, img)
    ...
    if(size(img,3)==3)
        ...
        img_dim = [img_y_size, img_x_size];
        Cr = imgr(sub2ind(img_dim, round(q_y), round(q_x)));
        Cg = imgg(sub2ind(img_dim, round(q_y), round(q_x)));
        Cb = imgb(sub2ind(img_dim, round(q_y), round(q_x)));
